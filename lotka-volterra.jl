...
```math
\left \{ \begin{aligned}
\dot{u}_1 \left ( t \right ) & = u_1 \left ( t \right ) \left [ \alpha - \beta u_2 \left ( t \right ) \right ], \\
\dot{u}_2 \left ( t \right ) & = u_2 \left ( t \right ) \left [ \delta u_1 \left ( t \right ) - \gamma \right ]
\end{aligned} \right .
```
où

* ``t`` est la variable indépendante (le temps) ;
* ``u_1 \left ( t \right )`` est l'effectif des proies à l'instant ``t`` ;
* ``u_2 \left ( t \right )`` est l'effectif des prédateurs à l'instant ``t``.
