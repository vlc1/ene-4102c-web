...
```math
T'' \left ( x \right ) = \sin \left ( \pi x \right )
```
avec les conditions
```math
\left \{ \begin{aligned}
...
1. Réécrire l'équation scalaire ci-dessus sous la forme d'un système de deux EDO couplées de la forme
```math
\left \{
\begin{aligned}
q _ 1 ' \left ( x \right ) & = f _ 1 \left ( x, \vec{q} \right ), \\
q _ 2 ' \left ( x \right ) & = f _ 2 \left ( x, \vec{q} \right ).
\end{aligned}
\right .
```
Implémenter ce modèle dans la fonction `source` ci-dessous.
...
md"""
La spécification des conditions aux limites est plus complexe. On rappelle qu'afin de considérer des conditions générales, de types (``c \in \left \{ a, b \right \}``, les bornes du domaines):

- Dirichlet, telle que ``T \left ( c \right ) = 1`` ;
- Neumann, telle que ``T' \left ( c \right ) = 0`` (homogène) ;
- Robin, telle que ``T \left ( c \right ) - \pi T' \left ( c \right ) = 0`` (homogène) ;
- Périodiques, telles que ``T \left ( a \right ) = T \left ( b \right )`` ;

on représente l'information par la définition implicite
```math
h \left ( u, v \right ) = 0
```
où ``u`` contient la variable indépendante à gauche (``x = a``), et ``v`` à droite (``x = b``).

2. Modifier la fonction `bc!` ci-dessous afin de décrire les conditions aux limites
```math
\left \{ \begin{aligned}
T \left ( 0 \right ) & = 1, \\
T' \left ( 1 \right ) & = 0.
...
# ╔═╡ b32b2696-e515-4190-b1f4-53a76d1b0f6d
md"""
La fonction `shooting` ci-dessous implémente la résolution de problèmes aux limite par la méthode de tir.

3. Vérifier que les résultats numériques sont corrects.
...
```
On prendra pour valeurs ``\kappa _ 1 = 1`` et ``\kappa _ 2 = 2``. Cette EDO appartient à la catégorie des *multipoint boundary value problems*.

1. Réécrire ce système sous la forme d'un problème aux limites.
2. Réécrire le système d'EDO d'ordre 2 obtenu EDO d'ordre 2 sous la forme d'un système d'EDO d'ordre 1.
3. Résoudre ce problème numériquement et visualiser la solution.

"""

# ╔═╡ 0d7ed618-6d9a-4569-94f7-2e17884a5a18
# Question 3a
function ice_source(x, q)
	res = similar(q)
	res[1] = 0
	res[2] = 0
	res[3] = 0
...
# ╔═╡ 9a87e296-9916-495d-8294-74f2cf354c5b
md"""
4. Calculer le flux de chaleur en ``x = 1`` et vérifiez qu'il est bien égal à celui en ``x = 0``.
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "9d79175f015054424db57080c698f9092248a0bc"
